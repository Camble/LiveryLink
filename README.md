...
[![Download][download]](https://github.com/Camble/LiveryLink/releases/download/v0.32.2/LiveryLink.v0.32.2.zip)
...
- Fixed an issue that prevented Cloudflare challenge from displaying

## 0.32.1

- Improved reliability of new scraper
...
- Added: F-100D
- Fixed: Error when trying to install livery from `files.digitalcombatsimulator.com`

## 0.31.6

- Further livery updater tweaks

## 0.31.5

- Added: "LiveryLink" user-agent to all outbound HTTP requests

## 0.31.4

- Fixed: Duplicate update checks for livery packs

## 0.31.3

- Improved livery update process to reduce network traffic

## 0.31.2

- Added a small delay between requests when checking liveries for updates.

## 0.31.1

- Implemented workaround for times when User Files returns `403 Forbidden`
...
- Fixed: Livery Link doesn't start if `Liveries` folder does not exist.
- Fixed: Google Drive liveries are not updated automatically.
...
- Fixed: UI `Not Responding` during livery extraction.

## 0.29.26

- Fixed: `OverflowException` on startup.

## 0.29.25

- Added `La-7` support.
- Improved Aircraft icon detection for community mods.


## 0.29.24

- Added `Mirage 2000D` community mod support.

## 0.29.23

- Fixed detection of `F-5E Remastered` module.
- Fixed `F-5E Remastered` icon.

## 0.29.22

- Fixed: Liveries compatible with variants don't prompt to select a module if the Livery author's archive structure already has the module folder.
...
- Removed the `Delete Forever` option for manually installed liveries - this is intended for received shares only.
...
- Livery update finds livery by name instead of folder, to account for folder structure changes.
...
- Consolidated livery update error notifications to prevent spam.

## 0.29.9

- Ensures livery updates are successful before deleting temporary files, to prevent unnecessary repeat downloads.

## 0.29.8

- Additional logging to improve livery update process.

## 0.29.7

- Downgrade compression library to workaround an issue with RAR extraction.

## 0.29.6

- Re-worded the livery update prompt to include a count of liveries to be re-downloaded.

## 0.29.5

- Livery update process tweaks.

## 0.29.4

- Added some logging where liveries cannot be updated, to help with diagnosis.
...
- Fixed: A single `description.lua` file without any texture files is still detected as a livery and extracted

## 0.28.6

- Fixed: `Sequence contains no elements` when installing liveries for certain modules

## 0.28.5

- Fixed: Duplicate aircraft names listed in the selection dialog
- With `Install liveries for compatible modules` enabled, no longer prompt for aircraft selection when multiple possible matches are equivalent (e.g. `F-5E-3` & `F-5E_FC`)

## 0.28.4

- Fixed: Auto-updater fails to update the DCS script

## 0.28.3

- Fixed a small bug in the naming of a livery share, if installed with a very old version of Livery Link
- Fixed an issue where the client would attempt a re-connect to the server when it should just disconnect
- Some logging tweaks

## 0.28.2

- Updated dependencies
- Made more improvements to reduce risk of false AV positives

## 0.28.1

- Stripped metadata from all embedded PNG resources
...
- Removed self-signed certificate to aid with false positive malware detection

## 0.27.25

- Fixed false positive malware detection
...
- Added some specific logging to help diagnose an intermittent sharing bug

## 0.27.19

- Fixed an error when displaying notification on receiving shared liveries

## 0.27.18

- Fixed an issue with case-sensitive folder names, which intermittently prevented the sending of shared liveries

## 0.27.17

- Fixed an issue with module detection, which prevented the download of shared liveries

## 0.27.16

- Fixed error in Livery importer when `liveries` folder does not exist.

## 0.27.15

- On first run, Livery Link will now detect and import any existing liveries it finds. This process can also be run manually from the `Liveries` menu.
...
- Added: User option to toggle log file generation

## 0.27.12

- Fixed: Updater doesn't launch on exit if Livery Link was launched automatically alongside DCS
...
- Added OH-6A module support

## 0.27.8

- Detect if running inside a ZIP file and exit

## 0.27.7

- Fixed: Noticeable delay on startup before displaying UI, while checking for updates

## 0.27.6

- Another attempt to fix a strange bug preventing the updater from launching on exit

## 0.27.5

- Fixed: Updater not launched correctly on exit

## 0.27.4

- Fixed: Updater doesn't extract when run manually

## 0.27.3

- Added Mig-29A Fulcrum support
- No longer back up updater during auto-update

## 0.27.2

- Fixed: `Install Mod` sometimes fails because DCS is still closing
- Minor logging improvements

## 0.27.1

- Fixed: Livery Link hangs during client auto-reconnect
- Fixed: Receive "Player has downloaded your livery" notification even if download was skipped due to user settings.

## 0.27.0

- Added the ability to install liveries from nested archives
